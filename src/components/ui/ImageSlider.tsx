...
export const ImagesSlider = ({
  ...
  direction = "up",
    ...
    //update the image props and add imageProps
    images: {
        src: string;
        alt?: string;
      }[];
  ...
  const loadImages = () => {
    ...
    const loadPromises = images.map((image) => {
      return new Promise((resolve, reject) => {
        //use 'window.Image' for clarity by changing to new window.Image
        const img= new window.Image();
        //update img to img.src and resolve(image) to (image.src)
        img.src = image.src;
